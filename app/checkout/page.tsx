...
import { OrderSuccessModal } from './completeMessage';
...
export default function CheckoutPage() {
  ...
  const handleCompleteOrder = async () => {
    ...
    // 2. Cart clear karne se PEHLE current total ko save kar lein
    setFinalTotal(total.toFixed(1));
    setFinalOrderId(randomId);

    setIsProcessing(false);
    setShowModal(true);

    // Ab cart clear karne se modal ki value par farq nahi parega
    clearCart();
  };
  ...
  return (
    <>
      <section className={`${poppins.className} bg-white min-h-screen px-5 md:pt-20 pt-10 md:px-12 lg:px-20 md:mb-20 mb-10`}>
        <div className="max-w-[1200px] mx-auto paymentChecked  flex flex-col-reverse md:flex-col  md:grid lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  />
                  <label
                    className={`flex items-center gap-3 text-[15px] font-[400] rounded-xl px-3 py-2 transition-colors ${saveInfo ? ' text-black' : 'text-black'
                      }`}
                  >
                    <input
                      type="checkbox"
          ...
          <div className="pt-3">
            <div className="flex justify-end md:mb-10">
              <div className="flex justify-between w-full h-fill md:mb-auto mb-9">
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-4 text-[16px] font-[400]">
                  <div className="flex items-center md:gap-4 gap-2">
                    <div className="md:w-25 md:h-25 w-17 h-17 relative rounded-md overflow-hidden md:bg-[#F5F3ED] p-1">
                      <Image src={item.image} alt={item.name} fill className="object-contain" />
                    </div>
                    <div>
                      <p className="text-black md:text-[18px] text-[14px] font-[700]">{item.name}</p>
                      <p className="text-[#3B3B3B] md:text-[15px] text-[12px] font-[400] md:max-w-[290px] max-w-[200px]">
